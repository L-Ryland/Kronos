...
def infer_timestamp_step(timestamps):
    """Infer the candle interval from timestamp data."""
    timestamp_series = pd.Series(pd.to_datetime(timestamps)).dropna()
...
def prepare_prediction_inputs(df, required_cols, lookback, pred_len, start_date=None):
    """Build model inputs for either future forecast or historical comparison."""
    if start_date:
        ...
        if len(matching_positions) == 0:
            raise ValueError(
                f'No data found from start time {start_dt.strftime("%Y-%m-%d %H:%M")}'
            )

        historical_start_idx = int(matching_positions[0])
        time_range_df = df.iloc[historical_start_idx:]
        ...
        context_df = time_range_df.iloc[:lookback]
        ...
        prediction_type = (
            f"Kronos model historical comparison (selected window: first {lookback} "
            f"data points for prediction, next {pred_len} data points for comparison, "
            f"time span: {time_span})"
        )
    else:
        historical_start_idx = max(0, len(df) - lookback)
        context_df = df.iloc[historical_start_idx:]
        actual_df = None

        last_timestamp = context_df["timestamps"].iloc[-1]
        time_step = infer_timestamp_step(df["timestamps"])
        y_timestamp = pd.Series(
            pd.date_range(
                start=last_timestamp + time_step, periods=pred_len, freq=time_step
            ),
            name="timestamps",
        )
        prediction_type = (
            f"Kronos model future forecast (latest {lookback} data points, "
            f"next {pred_len} data points)"
        )
...
def prediction_timestamps_from(pred_df, fallback_timestamps):
    """Prefer the model output index when it carries real datetimes."""
    if pred_df is not None and len(pred_df) > 0:
        pred_index = pred_df.index
        if pd.api.types.is_datetime64_any_dtype(pred_index):
            return pd.Series(pred_index, name="timestamps")

    return pd.Series(fallback_timestamps, name="timestamps")
...
def create_prediction_chart(
    ...
    if pred_df is not None and len(pred_df) > 0:
        if pred_timestamps is not None:
            pred_timestamps = pd.Series(pred_timestamps)
        elif pd.api.types.is_datetime64_any_dtype(pred_df.index):
            pred_timestamps = pd.Series(pred_df.index)
            ...
            time_diff = infer_timestamp_step(df["timestamps"])
...
def vendor_plotly():
    """Serve Plotly locally so charts do not depend on the CDN."""
    ...
    try:
        ...
        if MODEL_AVAILABLE and predictor is not None:
            try:
                ...
                prediction_inputs = prepare_prediction_inputs(
                    df, required_cols, lookback, pred_len, start_date
                )
                x_df = prediction_inputs["x_df"]
                x_timestamp = prediction_inputs["x_timestamp"]
                y_timestamp = prediction_inputs["y_timestamp"]
                actual_df = prediction_inputs["actual_df"]
                historical_start_idx = prediction_inputs["historical_start_idx"]
                prediction_type = prediction_inputs["prediction_type"]
        ...
        pred_timestamps = prediction_timestamps_from(pred_df, y_timestamp)
        ...
        for i, (_, row) in enumerate(pred_df.iterrows()):
            prediction_results.append(
                {
                    "timestamp": (
                        pred_timestamps.iloc[i].isoformat()
                        if i < len(pred_timestamps)
                        else f"T{i}"
                    ),
                    "open": float(row["open"]),
                    "high": float(row["high"]),
